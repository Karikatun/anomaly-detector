...
Реализован авторитетный серверный модуль прежнего набора правил: создание партии, скрытая конфигурация, выбор слотов, распределение мощности, разведка, лаборатория, анализ модели, пять раундов контрактов, финальная Scientific Model, Budget, Corporate Trust, Rating, идемпотентность запросов, participant-only audit и восстановление состояния из PostgreSQL. Есть парольный вход, профиль с locale и удалением аккаунта, приватные комнаты, готовность игроков и запуск заполненной комнаты после подтверждения всего состава, история матчей, HTTP/WebSocket-realtime с переподключением и рабочий экран партии.

Yandex ID подключён через provider-agnostic OAuth-основу с PKCE; подключение VK ID остаётся в работе. Согласован новый набор игровых правил, но его серверная и UI-миграция ещё не началась: [GAME_DESIGN_BRIEF.md](docs/GAME_DESIGN_BRIEF.md), [MVP_IMPLEMENTATION_PLAN.md](docs/MVP_IMPLEMENTATION_PLAN.md). Развёртывание пока не выполняется; целевая инфраструктура — Yandex Cloud через `yc` CLI.
...
- If the user explicitly chooses Yandex Cloud, use [docs/YANDEX_CLOUD.md](docs/YANDEX_CLOUD.md): a Compute Cloud instance group for API/worker, Application Load Balancer for HTTPS/WebSocket ingress, Managed Service for PostgreSQL for production data, Object Storage for files/static sites, and Cloud CDN for public static/media delivery.
...
Test runners use the separate Docker Compose `postgres_test` service and the `TEST_DATABASE_URL` shape from `.env.example`/`backend/.env.example`. Webapp Playwright E2E starts `postgres_test`, applies migrations to `anomaly_detector_test`, runs the browser flow, and tears down its test database volume by default.
...
- Yandex Cloud path: [Yandex Cloud CLI](https://yandex.cloud/en/docs/cli/quickstart), [Compute Cloud](https://yandex.cloud/en/docs/compute/), [Application Load Balancer](https://yandex.cloud/en/docs/application-load-balancer/), [Yandex Container Registry](https://yandex.cloud/en/docs/container-registry/quickstart), [Yandex Managed PostgreSQL](https://yandex.cloud/en/docs/managed-postgresql/), [Yandex Managed Service for Valkey](https://yandex.cloud/en/docs/managed-redis/), [Yandex Object Storage static hosting](https://yandex.cloud/en/docs/storage/operations/hosting/setup), [Yandex Object Storage AWS CLI](https://yandex.cloud/en/docs/storage/tools/aws-cli), [Yandex Cloud CDN](https://yandex.cloud/en/docs/cdn/concepts/), and [Yandex Cloud Marketplace Image Resizer](https://yandex.cloud/en/marketplace/products/yc/image-resizer)
