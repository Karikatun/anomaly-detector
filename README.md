...
В реализации остаются подключение Yandex ID и VK ID, приватные комнаты, realtime и интерфейс партии. Развёртывание пока не выполняется; целевая инфраструктура - Yandex Cloud через `yc` CLI.
