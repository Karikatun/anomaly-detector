...
Реализован авторитетный серверный модуль тендера: создание партии, скрытая конфигурация, выбор слотов, распределение мощности, разведка, лаборатория, анализ модели, пять раундов обычных контрактов, Budget, Corporate Trust, идемпотентность запросов, participant-only audit и восстановление состояния из PostgreSQL.

В реализации остаются Final Contract, итоговое начисление Rating и Scientific Model, комнаты и интерфейс партии. Развёртывание пока не выполняется; целевая инфраструктура - Yandex Cloud через `yc` CLI.
