...
В проект входят публичный Astro-сайт, игровой React-клиент, отдельный операторский React-клиент, backend на Bun и Hono, PostgreSQL и общие Zod-контракты. Текущая операторская реализация read-only; для Production MVP утверждены только узкие аудируемые команды почтовой политики и обработки обратной связи. Серверное ядро целевого набора правил реализовано; игровой интерфейс и подготовка Public MVP Journey продолжаются.
