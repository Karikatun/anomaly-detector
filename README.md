...
Реализован авторитетный серверный модуль тендера: создание партии, скрытая конфигурация, выбор слотов, распределение мощности, разведка, лаборатория, анализ модели, пять раундов контрактов, финальная Scientific Model, Budget, Corporate Trust, Rating, идемпотентность запросов, participant-only audit и восстановление состояния из PostgreSQL. Подготовлена provider-agnostic OAuth-основа с PKCE и одноразовыми транзакциями; внешние OAuth-провайдеры пока не включены.

В реализации остаются подключение Yandex ID и VK ID, вход в приватные комнаты, старт партии, realtime и интерфейс партии. Развёртывание пока не выполняется; целевая инфраструктура - Yandex Cloud через `yc` CLI.
