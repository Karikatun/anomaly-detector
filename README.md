...
Играть: [anomaly-detector.ru](https://anomaly-detector.ru).
...
В проект входят публичный Astro-сайт, игровой React-клиент, отдельный операторский React-клиент, backend на Bun и Hono, PostgreSQL и общие Zod-контракты. Операторская поверхность закрыта по умолчанию: системный обзор остаётся read-only, а изменения ограничены узкими аудируемыми командами почтовой политики и обработки обратной связи. Серверное ядро целевого набора правил реализовано; игровой интерфейс и подготовка Public MVP Journey продолжаются.
...
- [карта аудитов и проверок](docs/AUDIT_GUIDE.md), [политика безопасности](SECURITY.md), [CI и защита веток](docs/CI.md);
...
Перед запуском backend освобождает порты API и worker health (`3000` и `3001` по умолчанию), но завершает только процессы из каталога этого backend. Если порт занят посторонним приложением, запуск останавливается с явной ошибкой вместо принудительного завершения чужого процесса.
