...
if (process.env.TEST_SKIP_DOCKER !== '1') {
  ...
  await waitForComposePostgres('postgres_test', 'the_game_test', env)
}

run('bun', ['run', 'prisma:generate'], { env })
run('bun', ['run', 'prisma:deploy'], { env })
run('bun', ['test', 'src/modules/auth/auth.integration.test.ts', 'src/modules/tender/tender.integration.test.ts', 'src/modules/tender/realtime/realtime.integration.test.ts'], { env })
