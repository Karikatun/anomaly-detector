...
describe('stop-dev-ports preflight', () => {
  test('stops backend-owned listeners on both configured development ports', async () => {
    ...
    const result = await runPreflight(apiPort, workerPort, commandPath)
  ...
  test('refuses to stop a listener outside the backend workspace', async () => {
    ...
    const result = await runPreflight(foreignPort, foreignPort + 1, commandPath)
...
async function runPreflight(apiPort, workerPort, commandPath) {
