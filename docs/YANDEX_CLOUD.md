...
Обе Caddy-конфигурации блокируют скрытые файлы, резервные копии, архивы,
дампы, ключи и source maps до поиска файла и SPA fallback. Исключение для
`/.well-known/` сохраняет обычные публичные ресурсы и ACME challenge;
вложенные скрытые файлы и резервные копии там тоже закрыты. API не раздаёт
файлы из репозитория. Эти правила начинают действовать на сервере только
после публикации и проверки обновлённой Caddy-конфигурации.

Сборки `website`, `webapp` и `adminapp` автоматически проверяют готовый
выходной каталог, включая нестандартный `outDir`. Проверка отклоняет
служебные файлы и символические ссылки, а также ищет известные форматы
секретов внутри файлов. Она не использует исключения Git-сканера для `dist`
и не выводит найденные значения. Отсутствующий или неполный каталог не
считается безопасным. Ошибка останавливает сборку, но оставшийся результат
на диске нельзя публиковать.

Перед загрузкой проверьте именно подготовленные каталоги публикации, а не
исходный checkout:

```bash
bun run security:artifacts /absolute/website/dist /absolute/webapp/dist /absolute/adminapp/dist
bun run test:static-serving
```

В первой команде укажите только публикуемые поверхности. Вторая проверяет
настоящий Caddy по обеим версиям конфигурации с синтетическими файлами;
нужен локальный Docker и образ, закреплённый в
`scripts/static-serving.smoke.test.mjs`. Тест использует только временные
каталоги и порты на loopback. Он не доказывает, что production уже обновлён.
Каталоги публикации после проверки должны оставаться неизменными; секреты
и резервные копии храните вне них. Поиск известных форматов токенов не
заменяет проверку того, какие значения передаются в frontend environment.
