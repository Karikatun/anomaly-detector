...
This is the maintained alternative runbook for an explicitly requested
DigitalOcean deployment. The current Anomaly Detector production path is Yandex
Cloud; start from [DEPLOYMENT.md](DEPLOYMENT.md) and
[YANDEX_CLOUD.md](YANDEX_CLOUD.md) unless the user has deliberately selected
DigitalOcean.
...
- whether mobile is active and included in this release;
- whether an external CDN is required for advanced bot, rate-limit, or geographic traffic controls.

Local setup from `README.md` and [LOCAL_DATABASE.md](LOCAL_DATABASE.md) does not
require DigitalOcean credentials.
...
Use worker components only after a real long-running handler exists. The generator requires `DO_BACKEND_WORKER_RUN_COMMAND` and refuses the placeholder command `bun run start:worker`, because that placeholder exits immediately and should not be deployed as an App Platform worker. Production should schedule `maintenance:cleanup`; it removes stale authentication data and waiting rooms older than 24 hours. Keep every schedule at DigitalOcean's supported cadence of at least 15 minutes. Both optional components use `backend/Dockerfile`, the repository-root build context, and the same managed PostgreSQL binding as the API. Add Spaces or other runtime secrets to those components when the specific background task needs them.
...
When cross-instance Tender delivery needs Pub/Sub, create a DigitalOcean Managed Valkey cluster in the same region as the app and database, attach the connection string to the backend as a runtime secret such as `VALKEY_URL`, and keep it out of static-site build-time env. Do not enable Valkey in the baseline deployment until horizontal scaling requires it.
...
The minimum sufficient website tier is Static Site only. The current Astro surface is the public Anomaly Detector landing page and uses rebuild/redeploy for durable content changes. Keep authentication, rooms, Tender, profile, history, and other interactive player workflows in `webapp`.
...
## Failure Modes This Runbook Guards Against
