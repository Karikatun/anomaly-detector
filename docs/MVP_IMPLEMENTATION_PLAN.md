...
9. [x] Проверить initial HTML, социальные превью, structured data, robots,
...
1. [x] Добавить в профиль карточку «Защита аккаунта» со всеми состояниями из
   ADR 0005: отсутствует, ожидает кода, 24-часовой период защиты, активен,
   выполняется замена, управляется Яндекс ID, сервис заблокирован.
...
4. [x] Замена активного Recovery Email требует текущий пароль и коды со старого
   и нового адресов; старый адрес действует до обоих подтверждений. После
   замены отозвать остальные сессии и существующие recovery credentials.
5. [x] После активации выдать один раз восемь Recovery Code: копирование,
...
6. [x] Recovery Code позволяет восстановить пароль или заменить недоступный
...
7. [x] Не давать support или администратору команды назначения email, просмотра
...
1. [x] Форма принимает login, а не email, и возвращает одинаковый внешний
   результат для неизвестного login, отсутствующей почты, Yandex-only аккаунта,
   rate limit и принятого запроса.
2. [x] Отправлять 15-минутную одноразовую ссылку с длинным случайным токеном;
...
3. [x] Страница сброса не загружает аналитику или сторонние ресурсы и задаёт
   `Referrer-Policy: no-referrer`.
4. [x] Успешный сброс атомарно отзывает все сессии, ссылки, почтовые коды и
   Recovery Code, отправляет security-уведомление и не создаёт новую сессию.
5. [x] Применить отдельные login/IP budgets из ADR 0013 и проверить race двух
...
1. [x] Добавить отдельный пункт меню «Обратная связь» и собственную форму, а не
   ...
   каналы.
2. [x] Для ошибки собирать описание, шаги, ожидание и возможность продолжить;
...
3. [x] Не принимать вложения, сырые логи, полный URL, IP, токены или игровое
   состояние. Контакт и связь с аккаунтом — два отдельных добровольных действия;
   Recovery Email автоматически не использовать.
4. [x] Сохранять Feedback Report в PostgreSQL и возвращать копируемый публичный
   номер. Лимитировать до 5 принятых обращений в сутки на аккаунт и 20 на IP.
5. [x] Добавить в adminapp очередь и только перечисленные команды: взять в
...
6. [x] Удалять `new`/`in_review` не позднее 180 дней, а resolved/rejected,
   добровольный контакт и исходник перенесённой задачи — через 30 дней.
   В MVP месячные counts не формируются; при отдельном будущем включении их
   retention ограничен 13 месяцами.
7. [x] Не создавать «Мои обращения» в MVP. Без добровольного контакта канал
