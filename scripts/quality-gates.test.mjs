...
test('defines fast commit and complete push quality gates', () => {
  ...
  expect(packageJson.scripts['test:tooling']).toStartWith('bun run security:skills && ')
  expect(packageJson.scripts['test:tooling']).toContain('scripts/verify-skills.test.mjs')
  expect(packageJson.scripts['test:tooling']).toContain('bun run test:skills')
